...
async function pushToGitHub() {
  try {
    ...
    try {
      ...
      const commit = await makeGitHubRequest('GET', `/repos/${REPO_OWNER}/${REPO_NAME}/git/commits/${currentSha}`);
      if (commit && commit.tree && commit.tree.sha) {
        baseTreeSha = commit.tree.sha;
        console.log(`🌲 Base tree SHA: ${baseTreeSha}`);
      } else {
        console.log('⚠️  Could not get tree SHA from commit, creating without base tree');
      }
    } catch (err) {
      console.log('🆕 Branch does not exist or error accessing it, will create new branch');
      console.log(`   Error: ${err.message}`);
    }

    // Create tree entries for all files (process in smaller batches to avoid payload size issues)
    ...
    for (const file of files) {
      ...
      if (isBinaryFile(file)) {
        ...
        // For text files, decode from base64 back to string
    ...
    console.log(`📋 Created ${treeEntries.length} tree entries`);
    ...
    // Only add base_tree if we have it
    if (baseTreeSha) {
...
Pushed from Replit Agent`,
    ...
    // Only add parents if we have a current commit
    if (currentSha) {
      commitData.parents = [currentSha];
    }
    ...
    if (currentSha) {
      ...
      await makeGitHubRequest('PATCH', `/repos/${REPO_OWNER}/${REPO_NAME}/git/refs/heads/${BRANCH}`, {
        sha: newCommit.sha,
        force: false // Don't force update to avoid overwriting changes
      });
    } else {
      // Create new branch
